...
int isValidMitoSlice_25d(snake25d *s, int slice, dataPacket *dp) {
	...
	z1 = (dp->ces_lo->start_z > dp->ces_hi->start_z) ?
			dp->ces_lo->start_z : dp->ces_hi->start_z;
	...
	z2 = (dp->ces_lo->end_z < dp->ces_hi->end_z) ?
			dp->ces_lo->end_z : dp->ces_hi->end_z;
	...
	if (z1 != s->start_z || z2 != s->end_z) {
		printf("Datapacket does not match!\n");
		return 0;
	}
	...
	int n[] = { SN_N };
	IplImage *region[SN25D_T];
	for (sl = start; sl <= end; sl++) {
		region[sl] = cvCreateImage(
				cvSize(dp->ces_lo->ces[sl + sl_ces_lo]->w,
						dp->ces_lo->ces[sl + sl_ces_lo]->h), 8, 1);
		cvSetZero(region[sl]);
	}
	CvPoint p[SN25D_T][SN_N];
	CvPoint *pa;
	for (sl = start; sl <= end; sl++) {
		for (i = 0; i < SN_N; i++) {
		...
		pa = p[sl];
		cvFillPoly(region[sl], &pa, n, 1, cvScalar(255));
	}
	h_range = 0;
	start = end = slice;
	range = 1;
	...
	for (sl = start; sl <= end; sl++) {
		for (i = 0, ip = SN_N - 1, in = 1; i < SN_N;
			...
			j = p[sl][i].y * dp->grad_ces_lo->grad_ces[sl + sl_grad_ces_lo]->w
					+ p[sl][i].x;
			gx = dp->grad_ces_lo->grad_ces[sl + sl_grad_ces_lo]->grad_ce[j][0];
			gy = dp->grad_ces_lo->grad_ces[sl + sl_grad_ces_lo]->grad_ce[j][1];
			m = sqrt(gx * gx + gy * gy);
			Eboundary[sl][i] = 0;
			if (m > EPS) {
				j = p[sl][i].y * dp->ces_lo->ces[sl + sl_ces_lo]->w
						+ p[sl][i].x;
	...
	for (sl = start; sl <= end; sl++) {
		...
		int cx, cy;
		for (i = 0; i < dp->cps_hi->cps[sl + sl_cps_hi]->len; i++) {
			cx = cvRound(dp->cps_hi->cps[sl + sl_cps_hi]->cx[i] * xscale);
			cy = cvRound(dp->cps_hi->cps[sl + sl_cps_hi]->cy[i] * yscale);
			if (cvGetReal2D(region[sl], cy, cx)) {
				j = dp->cps_hi->cps[sl + sl_cps_hi]->cy[i]
						* dp->ces_hi->ces[sl + sl_ces_hi]->w
						+ dp->cps_hi->cps[sl + sl_cps_hi]->cx[i];
	...
	if (DT_REPORT) {
		printf("------------\n");
		printf("Eboundary = %f %c\n", avgEboundary, (c1 ? ' ' : '-'));
		printf("Eregion = %f %c\n", avgEregion, (c2 ? ' ' : '-'));
		printf("Maxgap = %f (%f nm) %c\n", maxGap,
				maxGap * TFACTOR * LE_SSIZE_LO, (c3 ? ' ' : '-'));
		printf("Totalgap = %f (%f nm) %c\n", totalGap,
				totalGap * TFACTOR * LE_SSIZE_LO, (c4 ? ' ' : '-'));
		printf("Gap ratio = %f %c\n", gapRatio, (c5 ? ' ' : '-'));
		printf("Bordergap ratio = %f %c\n", borderGapRatio, (c6 ? ' ' : '-'));
		printf("Gap count = %d\n", countGap);
		printf("Bordergap count = %d %c\n", countBorderGap, (c7 ? ' ' : '-'));
		printf("Curvature Local = %f %c\n", curvL, (c8 ? ' ' : '-'));
		printf("Curvature Global = %f %c\n", curvG, (c9 ? ' ' : '-'));
		printf("Area = %f (%f nm2) %c\n", avgArea,
				avgArea * (TFACTOR * TFACTOR) * (LE_SSIZE_LO * LE_SSIZE_LO),
				((c14 || c15) ? ' ' : '-'));
		printf("Circumference = %f (%f nm)\n", avgCircum,
				avgCircum * TFACTOR * LE_SSIZE_LO);
		printf("Major axis = %f (%f nm) %c\n", avgMajorAxis,
				avgMajorAxis * TFACTOR * LE_SSIZE_LO, (c11 ? ' ' : '-'));
		printf("Minor axis = %f (%f nm) %c\n", avgMinorAxis,
				avgMinorAxis * TFACTOR * LE_SSIZE_LO, (c16 ? ' ' : '-'));
		printf("Min. axis = %f (%f nm) %c\n", avgMinAxis,
				avgMinAxis * TFACTOR * LE_SSIZE_LO, (c12 ? ' ' : '-'));
		printf("Min/Minor ratio = %f %c\n", avgMinMinorRatio,
				(c13 ? ' ' : '-'));
		printf("Max nc = %d %c\n", avgMaxNumCriticals, (c10 ? ' ' : '-'));
		printf("------------\n");
	}

	// Free mem.
	for (sl = start; sl <= end; sl++)
		cvReleaseImage(&region[sl]);
	//
	return is_Valid;
}
